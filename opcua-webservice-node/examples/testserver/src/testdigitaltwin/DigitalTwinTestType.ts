...
import { DigitalTwinType } from "opcua-webservice-node";
import { GetDescriptorRequest, GetDescriptorResponse, GetSubmodelsRequest, GetSubmodelsResponse } from "opcua-webservice-node";
import { SubmodelDescriptor } from "opcua-webservice-node";
...
export class DigitalTwinTestType extends DigitalTwinType {
    ...
    override async onGetDescriptor(request: GetDescriptorRequest): Promise<GetDescriptorResponse> {
        //console.log(`Get descriptor, ${request.id}`);
