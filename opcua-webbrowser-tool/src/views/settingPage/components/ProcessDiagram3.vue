...
<script setup>
import { ref, reactive, onMounted, onUnmounted, nextTick, watch,
computed } from 'vue'
import { ElMessage } from 'element-plus'
import { Refresh, ZoomIn, ZoomOut, RefreshRight } from
...
const props = defineProps({
  /**
   * 节点数据数组
   * 格式: [{ nodeId, name, type, display: { posX, posY, sizeX, sizeY 
}, value, status }, ...]
   */
  nodeDataList: {
    type: Array,
...
const nodes = computed(() => {
  ...
  return props.nodeDataList.map(item => {
    ...
    const node = {
      ...
      type: cached?.type || item.type || props.nodeTypeMap[nodeId] || 
'default',
      label: cached?.label || item.name || item.label || item.text || 
nodeId,
      x: display.posX || 0,
      y: display.posY || 0,
...
const getDefaultNodes = () => {
  ...
  const defaultNodes = [
    ...
    {
      id: 'valve2',
      type: 'butterfly-valve',  // 使用蝴蝶阀类型（两个相对的三角形）
      x: tankX + tankWidth / 2 + 40,  // 从储罐底部中心向右
      y: tankY + tankHeight + 30,     // 在储罐底部下方
      label: 'H002',
      width: 50,
      height: 40,
      nodeId: 'valve2'
    ...
    // FI 006 流量计：位于 P001 和 Sink0 之间管道的正上方
    {
      ...
      // P001 中心点: x = tankX + tankWidth + 90 + 30 = tankX + 
      //tankWidth + 120
      // Sink0 入口点: x = tankX + tankWidth + 260, y = tankY + 
      //tankHeight - 20 + 20 = tankY + tankHeight
      // 管道中点X: (P001右侧 + Sink0左侧) / 2
      // 管道Y坐标: tankY + tankHeight (水平管道)
      x: ((tankX + tankWidth + 90 + 60) + (tankX + tankWidth + 260)) / 
2 - 25,  // 中点X减去仪表宽度的一半
  ...
  const finalNodes = defaultNodes.map(node => {
    ...
    if (cached) {
      return {
        ...
        nodeId: cached.nodeId !== undefined ? cached.nodeId : 
node.nodeId,
...
const getDefaultLinks = () => {
  return [
    ...
    { from: 'valve1', to: 'tank' },
    { from: 'tank', to: 'valve2', isTankBottom: true }, 
     // 标记从Tank底部连接
    { from: 'valve2', to: 'pump' },
    { from: 'pump', to: 'sink0' }, 
     // 泵直接连接到Sink0，流量计在管道上方
    ...
    { from: 'flowmeter', to: 'pump', toMidpoint: 'sink0', isInstrument:
 true },
    // 电机连接到泵的管道
...
isTankBottom = false) => {
  ...
  if (fromNode.type === 'source') {
    ...
    // 储罐：判断是否从底部中心连接
    if (isTankBottom || (toNode.type === 'butterfly-valve' || 
toNode.label === 'H002')) {
      // 从底部中心连接
      fromX = fromCenterX
      fromY = fromNode.y + fromNode.height
    } else if (toCenterY > fromCenterY + 30) {
      // 出口：底部中心
      fromX = fromCenterX
      fromY = fromNode.y + fromNode.height
    } else {
  ...
  } else if (fromNode.type === 'butterfly-valve' || fromNode.label === 
'H002') {
    // 蝴蝶阀：从右侧连接
    fromX = fromNode.x + fromNode.width
    fromY = fromCenterY
  ...
  if (toNode.type === 'sink') {
    toX = toNode.x - 15
    toY = toCenterY
  } else if (toNode.type === 'butterfly-valve' || toNode.label === 
'H002') {
    // 蝴蝶阀：从左侧连接
    toX = toNode.x
    toY = toCenterY
...
const drawLinks = () => {
  ...
  links.value.filter(link => !link.isInstrument).forEach(link => {
    ...
    if (fromNode && toNode) {
      const isTankBottom = link.isTankBottom ||
                          (fromNode.type === 'tank' && (toNode.type ===
 'butterfly-valve' || toNode.label === 'H002'))
...
isTankBottom)
      ...
      // 判断是否需要90度拐弯（从Tank底部到H002）
      if (isTankBottom) {
        // 绘制90度折线：先向下，再向右
        drawPolyline(points.fromX, points.fromY, points.toX, 
points.toY, 'down-right')
      } else {
...
// 绘制90度折线
const drawPolyline = (x1, y1, x2, y2, direction = 'down-right') => {
  ctx.value.beginPath()
  ctx.value.moveTo(x1, y1)

  if (direction === 'down-right') {
    // 先向下到目标Y坐标，再向右到目标X坐标
    ctx.value.lineTo(x1, y2)  // 垂直向下
    ctx.value.lineTo(x2, y2)  // 水平向右
  } else if (direction === 'right-down') {
    // 先向右到目标X坐标，再向下到目标Y坐标
    ctx.value.lineTo(x2, y1)  // 水平向右
    ctx.value.lineTo(x2, y2)  // 垂直向下
...
const drawNode = (node) => {
  ...
  switch (node.type) {
    ...
    case 'butterfly-valve':
      drawButterflyValve(node, data)  // 新增：蝴蝶阀（两个相对的三角形）
      break
    case 'tank':
      drawTank(node, data)
      break
...
// 绘制蝴蝶阀（两个相对的三角形，符合P&ID标准）
const drawButterflyValve = (node, data) => {
  const { x, y, width, height } = node
  const statusColor = getStatusColor(data.status)
  const centerX = x + width / 2
  const centerY = y + height / 2
  const triangleWidth = width / 2 - 4  // 每个三角形的宽度
  const triangleHeight = height / 2 - 2  // 三角形的高度（半高）
  const gap = 3  // 两个三角形之间的间隙
  ...
  // 创建渐变
  const leftGradient = ctx.value.createLinearGradient(x, centerY - 
triangleHeight, centerX - gap, centerY)
  leftGradient.addColorStop(0, lightenColor(statusColor, 30))
  leftGradient.addColorStop(1, statusColor)

  const rightGradient = ctx.value.createLinearGradient(centerX + gap, 
centerY, x + width, centerY - triangleHeight)
  rightGradient.addColorStop(0, statusColor)
  rightGradient.addColorStop(1, lightenColor(statusColor, 30))

  // 左边三角形（指向右，尖端在中间）
  ctx.value.fillStyle = leftGradient
  ctx.value.strokeStyle = '#333'
  ctx.value.lineWidth = 2.5

  ctx.value.beginPath()
  ctx.value.moveTo(x, centerY - triangleHeight)          // 左上角
  ctx.value.lineTo(centerX - gap, centerY)               // 中心右侧（尖端）
  ctx.value.lineTo(x, centerY + triangleHeight)          // 左下角
  ctx.value.closePath()
  ctx.value.fill()
  ctx.value.stroke()

  // 右边三角形（指向左，尖端在中间）
  ctx.value.fillStyle = rightGradient

  ctx.value.beginPath()
  ctx.value.moveTo(x + width, centerY - triangleHeight)  // 右上角
  ctx.value.lineTo(centerX + gap, centerY)               // 中心左侧（尖端）
  ctx.value.lineTo(x + width, centerY + triangleHeight)  // 右下角
  ctx.value.closePath()
  ctx.value.fill()
  ctx.value.stroke()
  ...
  // 左三角形高光
  ctx.value.beginPath()
  ctx.value.moveTo(x + 3, centerY - triangleHeight + 5)
  ctx.value.lineTo(x + triangleWidth * 0.6, centerY - 3)
  ctx.value.lineTo(x + 3, centerY + 3)
  ctx.value.closePath()
  ctx.value.fill()

  // 右三角形高光
  ctx.value.beginPath()
  ctx.value.moveTo(x + width - 3, centerY - triangleHeight + 5)
  ctx.value.lineTo(x + width - triangleWidth * 0.6, centerY - 3)
  ctx.value.lineTo(x + width - 3, centerY + 3)
  ctx.value.closePath()
  ctx.value.fill()
  ...
  ctx.value.beginPath()
  ctx.value.arc(centerX, centerY, 3, 0, Math.PI * 2)
  ctx.value.fill()
}
...
const drawTank = (node, data) => {
  ...
  // 上端圆顶（椭圆）
  ctx.value.beginPath()
  ctx.value.ellipse(centerX, y, ellipseRadius, ellipseHeight, 0, 0, 
Math.PI * 2)
  ctx.value.fill()
  ctx.value.stroke()
  ...
  // 下端圆底（椭圆）
  ctx.value.beginPath()
  ctx.value.ellipse(centerX, y + height, ellipseRadius, ellipseHeight, 
0, 0, Math.PI * 2)
  ctx.value.fill()
  ctx.value.stroke()
  ...
  if (data.value) {
    ...
    if (levelHeight > 0) {
      ...
      if (levelHeight > ellipseHeight) {
        // 矩形部分
        ctx.value.fillRect(x + 5, liquidY, width - 10, levelHeight - 
ellipseHeight)
        ...
        // 只绘制圆底部分
        const angle = Math.acos((ellipseHeight - levelHeight) / 
ellipseHeight)
        ctx.value.beginPath()
        ctx.value.ellipse(
          centerX,
...
const drawPump = (node, data) => {
  ...
  ctx.value.fillStyle = 'rgba(255, 255, 255, 0.4)'
  ctx.value.beginPath()
  ctx.value.moveTo(triangleX - triangleSize * 0.3, triangleY - 
triangleSize * 0.3)
  ctx.value.lineTo(triangleX, triangleY)
  ctx.value.lineTo(triangleX - triangleSize * 0.3, triangleY + 
triangleSize * 0.3)
  ctx.value.closePath()
  ctx.value.fill()
}
...
const drawMotor = (node, data) => {
  ...
  // 创建渐变（模拟金属电机外壳）
  const gradient = ctx.value.createLinearGradient(x, y, x + width, y + 
height)
...
const drawInstrument = (node, data) => {
  ...
  ctx.value.lineWidth = 2.5
  ctx.value.beginPath()
  ctx.value.ellipse(centerX, centerY, radiusX, radiusY, 0, 0, Math.PI *
 2)
  ctx.value.fill()
  ctx.value.stroke()
...
const drawLabel = (node, data) => {
  ...
  // 阀门标签在下方
  if (node.type === 'valve' || node.type === 'butterfly-valve') {
    labelY = y + height + 12
  }
  // 仪表标签在下方
  ...
  if (data.value) {
    ...
    // 数值文本
    ctx.value.fillStyle = '#666'
    ctx.value.fillText(data.value, centerX, labelY + labelHeight / 2 + 
13)
  }
...
data.value.includes('%')) {
          ...
          data.value = `${Math.max(0, Math.min(100, num + 
(Math.random() - 0.5) * 10))}%`
        }
      }
    })
